...
def compute_and_store_risk(beach_id: str, activity_type: str) -> Optional[dict]:
    ...
    result = {
        ...
        "hard_override_reason": override_reason,
        "computed_at": datetime.now(timezone.utc).isoformat(),
        # BUG FIX: this was computed and stored in the DB row (line ~80, `explanation=
        # explanation.to_dict()`) but never included in the dict returned to the caller.
        # app/services/beach_service.get_beach_risk() reads result.get("explanation", {}),
        # which always fell back to {} -> top_factors always [] in the API response.
        "explanation": explanation.to_dict(),
    }
