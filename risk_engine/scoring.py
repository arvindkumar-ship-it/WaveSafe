...
class ScoreExplanation:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "hard_override_reason": self.hard_override_reason,
            "raw_score_pre_sigmoid": self.raw_score_pre_sigmoid,
            "top_factors": self.top_factors,  # BUG FIX: this key never existed before,
            # so app/services/beach_service.py's
            # result.get("explanation", {}).get("top_factors", []) always fell through
            # to the [] default — the API's RiskResponse.explanation.top_factors was
            # always empty regardless of what actually drove the score.
        }

    @property
    def top_factors(self, n: int = 3) -> list[str]:
        """Names of the n largest-magnitude contributing terms (weighted + interaction
        combined), biggest first — e.g. ["wave_height", "current_speed*wave_height"]."""
...
def compute_risk(
    ...
    # BUG FIX: coverage_gap is a bounded indicator (0.0 / 0.5 / 1.0 — see
    # features.coverage_gap()), not a normally-distributed forecast quantity like
    # wave_height/wind_speed. build_feature_window() deliberately hardcodes it to
    # 0.0 in the reference history window (it's "not forecast-driven"), which means
    # median=0 and IQR floors to EPSILON — so z-scoring it the same way as every
    # other feature turned any nonzero coverage_gap into a z-score in the hundreds
    # of thousands (e.g. 0.5 / (1e-6+1e-6) = 250000), which alone was enough to
    # saturate the sigmoid to exactly 1.0/"unsafe" regardless of actual conditions.
    # Fix: use its raw bounded value directly instead of z-scoring it.
    z = {}
