...
def load_latest_forecast(session, beach_id: str) -> Optional[BeachForecast]:
    """Returns the forecast slot closest to *now* — i.e. current conditions —
    not the furthest-out one in the table.

    BUG FIX: previously ordered by forecast_time DESC with no lower bound, which
    picked whichever row was farthest in the future (e.g. day 7 of a 7-day sync
    window) instead of the current/nearest-upcoming slot. That produced wildly
    wrong risk scores (e.g. risk_score=1 from an anomalous far-future wind gust,
    with hard_override_reason=null, days after the actual "now").
    """
    now = datetime.now(timezone.utc)

    # Prefer the nearest upcoming slot (>= now) — matches how the forecast rows
    # are generated (hourly slots going forward from ingestion time).
    stmt = (
        select(BeachForecast)
        .where(BeachForecast.beach_id == beach_id, BeachForecast.forecast_time >= now)
    ...
    # Fallback: every stored slot is already in the past (e.g. forecast wasn't
    # re-synced recently) — use the most recent one instead of returning None.
    stmt = (
        select(BeachForecast)
        .where(BeachForecast.beach_id == beach_id, BeachForecast.forecast_time < now)
        .order_by(BeachForecast.forecast_time.desc())
